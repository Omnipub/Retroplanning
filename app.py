...
# PostgreSQL Render si DATABASE_URL est configurée.
# SQLite sert de solution de secours en développement.  
database_url = os.environ.get(  
    "DATABASE_URL",  
    "sqlite:////tmp/retroplanning_blog.db"  
)
...
HOME_FAQS = [  
    {  
        ...
        "answer": (  
            "Un bon rétroplanning part de la date de l’événement et remonte "  
            "le temps. Définissez vos jalons clés — validation, production, "  
            "expédition — afin de visualiser vos dates limites."  
        ),  
    },  
    {  
        "question": "Qu’est-ce qu’un rétroplanning ?",  
        "answer": (  
            "Un rétroplanning est une méthode de planification qui part de "  
            "la date finale d’un projet pour identifier, dans l’ordre inverse, "  
            "toutes les étapes et échéances nécessaires."  
        ),  
    },  
    {  
        "question": "À qui s’adresse le générateur de rétroplanning ?",  
        "answer": (  
            "Il est conçu pour les agences de communication, organisateurs "  
            "d’événements, imprimeurs, chefs de projet et professionnels "  
            "de la production."  
        ),  
    },  
    {  
        ...
        "answer": (  
            "Le générateur permet de renseigner jusqu’à quatre étapes clés "  
            "avant la date finale de votre événement ou de votre livraison."  
        ),  
    },  
    {  
        "question": "Puis-je modifier les couleurs du graphique ?",  
        "answer": (  
            "Oui. Vous choisissez une couleur principale et une couleur "  
            "secondaire afin d’adapter le document à votre charte graphique."  
        ),  
    },  
    {  
        "question": "Puis-je utiliser le rétroplanning pour un projet autre qu’un événement ?",  
        "answer": (  
            "Oui. Il convient aussi aux projets de production, d’impression, "  
            "de lancement commercial, de logistique ou de livraison client."  
        ),  
    },  
    {  
        "question": "Le fichier est-il modifiable après téléchargement ?",  
        "answer": (  
            "Le téléchargement est un PNG HD prêt à être intégré dans une "  
            "présentation, un PDF ou un devis. Les données doivent être "  
            "modifiées dans le générateur avant l’export."  
        ),  
    },  
    {  
        "question": "Vais-je recevoir une facture pour mon entreprise ?",  
        "answer": (  
            "Oui. Après le paiement, vous pouvez générer une facture PDF "  
            "avec votre raison sociale et votre numéro de TVA."  
        ),  
    },  
]
...
def generate_png(  
    ...
    for i in range(len(steps) - 1):  
        ...
        color = c_main if i % 2 == 0 else c_alt
    ...
    footer = "  |  ".join(  
        item  
        for item in [footer_societe, phone, email, web]  
        if item  
    ) or nom_client
    ...
    filename = (  
        f"retroplanning_{nom_client.replace(' ', '_')}_"  
...
def generate_facture(order, client_info, num_facture):  
    ...
    story += [  
        ...
        Paragraph(  
            f"{VENDEUR['cp']} {VENDEUR['ville']}",  
            style  
        ),  
        Paragraph(  
            f"SIRET : {VENDEUR['siret']}",  
            style  
        ),  
        Paragraph(  
            f"N° TVA : {VENDEUR['tva']}",  
        ...
        Paragraph(  
            f"Email : {VENDEUR['email']} | Tél : {VENDEUR['tel']}",  
            style  
        ),  
        Spacer(1, 0.8 * cm),  
        Paragraph(  
            f"<b>FACTURE N° {num_facture}</b>",  
            ParagraphStyle(  
                "invoice",  
                fontSize=16,  
                textColor=green,  
        ...
        Paragraph(  
            f"<b>{client_info.get('raison_sociale', '')}</b>",  
            style  
        ),  
        Paragraph(  
            client_info.get("adresse", ""),  
            style  
        ),  
        Paragraph(  
            f"{client_info.get('cp', '')} {client_info.get('ville', '')}",  
            style  
        ),  
        Spacer(1, 0.8 * cm),  
    ]
    ...
    data = [  
        ...
        [  
            "Rétroplanning de production (PNG HD)",  
            "1",  
            f"{PRIX_HT:.2f} €",  
            f"20% ({TVA:.2f} €)",  
            f"{PRIX_TTC:.2f} €"  
    ...
    story += [  
        ...
        Spacer(1, 1 * cm),  
        Paragraph(  
            "Règlement effectué par PayPal. Merci pour votre confiance.",  
            ParagraphStyle(  
                "footer",  
                fontSize=9,  
...
def checkout():  
    ...
    params = urllib.parse.urlencode({  
        ...
        "item_name": (  
            f"Rétroplanning - {data['nom_evenement']} - "  
            f"{data['nom_client']}"  
        ),  
    ...
    return redirect(  
        f"https://www.paypal.com/cgi-bin/webscr?{params}"  
    )
...
def success():  
    ...
    order["png_path"] = generate_png(  
        ...
        order.get("email", ""),  
        order.get("web", ""),  
        order.get("footer_societe", ""),  
        order.get("c_main", "#3F8078"),  
        order.get("c_alt", "#75A097")  
    )
...
def download_png(token):  
    ...
    return send_file(  
        ...
        download_name=(  
            f"retroplanning_"  
            f"{order['nom_client'].replace(' ', '_')}.png"  
        )  
    )
...
def facture(token):  
    ...
    if request.method == "POST":  
        ...
        return send_file(  
            generate_facture(order, info, num),  
            as_attachment=True,  
...
def blog_category(category_slug):  
    articles = [  
        article  
        for article in Article.query.filter_by(  
            statut="publie"  
        ).order_by(  
            Article.published_at.desc()  
        ).all()  
...
def admin_generate():  
    ...
    if request.method == "POST":  
        ...
        if len(steps) < 2:  
            return (  
                "Il faut au moins 1 étape + la date d’événement.",  
                400  
            )
...
def save_article_from_form(article):  
    article.titre = request.form.get(  
        "titre",  
        ""  
    ).strip()

    article.slug = unique_slug(  
        request.form.get("slug", "").strip() or article.titre,  
        article.id  
    )

    article.categorie = request.form.get(  
        "categorie",  
        ""  
    ).strip() or "Guides"

    article.extrait = request.form.get(  
        "extrait",  
        ""  
    ).strip()

    article.resume = request.form.get(  
        "resume",  
        ""  
    ).strip()

    article.contenu = request.form.get(  
        "contenu",  
        ""  
    ).strip()

    article.points_cles = request.form.get(  
        "points_cles",  
        ""  
    ).strip()

    article.image_url = request.form.get(  
        "image_url",  
        ""  
    ).strip()

    article.image_alt = request.form.get(  
        "image_alt",  
        ""  
    ).strip()

    article.meta_title = request.form.get(  
        "meta_title",  
        ""  
    ).strip() or article.titre

    article.meta_description = request.form.get(  
        "meta_description",  
        ""  
    ).strip() or article.extrait
...
def admin_blog_new():  
    ...
    if request.method == "POST":  
        ...
        if (  
            not article.titre  
            or not article.extrait  
            or not article.contenu  
        ):  
            return render_template(  
                "admin_blog_form.html",  
                article=article,  
                error=(  
                    "Titre, extrait et contenu sont obligatoires."  
                )  
            )
...
@app.route(  
    "/admin/blog/<int:article_id>/modifier",  
    methods=["GET", "POST"]  
)  
...
def sitemap():  
    urls = [  
        ...
        (f"{SITE_URL}/formulaire", "monthly", "0.8"),  
    ...
    entries = "".join(  
        ...
        for loc, frequency, priority in urls  
    )

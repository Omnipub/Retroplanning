...
def generate_facture(order, client_info, num_facture):
    ...
    story += [
        Paragraph("<b>" + nom + "</b>",
                  ParagraphStyle("v", fontSize=14, textColor=green)),
        ...
        Paragraph("<b>FACTURE N " + num_facture + "</b>",
                  ParagraphStyle("t", fontSize=16, textColor=green, alignment=TA_CENTER)),
        Paragraph("Date : " + datetime.now().strftime("%d/%m/%Y"),
                  ParagraphStyle("d", fontSize=10, alignment=TA_CENTER)),
        Spacer(1, 0.5*cm),
        Paragraph("<b>Facture a :</b>",
                  ParagraphStyle("h", fontSize=11, textColor=green)),
    ...
    data = [
        ...
        ["Retroplanning de production (PNG HD)", "1",
         str(PRIX_HT) + " EUR", "20% (" + str(TVA) + " EUR)", str(PRIX_TTC) + " EUR"],
    ]
    ...
    story += [
        t, Spacer(1, 0.5*cm),
        Paragraph("<b>Total HT : " + str(PRIX_HT) + " EUR</b>",
                  ParagraphStyle("r1", fontSize=10, alignment=TA_RIGHT)),
        Paragraph("TVA 20% : " + str(TVA) + " EUR",
                  ParagraphStyle("r2", fontSize=10, alignment=TA_RIGHT)),
        Paragraph("<b>Total TTC : " + str(PRIX_TTC) + " EUR</b>",
                  ParagraphStyle("r3", fontSize=12, textColor=green, alignment=TA_RIGHT)),
        Spacer(1, 1*cm),
...
def facture(token):
    ...
    if request.method == "POST":
        info = {k: request.form.get(k, "") for k in
